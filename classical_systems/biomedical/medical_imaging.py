...
class MedicalImagingSimulation(SimulationBase):
    """Deterministic classical simulation for Medical Imaging."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Medical Imaging",
            parameters={"problem_size": 128, "seed": 21372, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 21372))
...
def main() -> dict[str, Any]:
    simulation = MedicalImagingSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("MedicalImagingSimulation complete:", output)
