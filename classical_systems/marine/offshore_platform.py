...
class OffshorePlatformSimulation(SimulationBase):
    """Deterministic classical simulation for Offshore Platform."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Offshore Platform",
            parameters={"problem_size": 128, "seed": 1660, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 1660))
...
def main() -> dict[str, Any]:
    simulation = OffshorePlatformSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("OffshorePlatformSimulation complete:", output)
