...
class HeatEngineSimulation(SimulationBase):
    """Deterministic classical simulation for Heat Engine."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Heat Engine",
            parameters={"problem_size": 128, "seed": 47821, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 47821))
...
def main() -> dict[str, Any]:
    simulation = HeatEngineSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("HeatEngineSimulation complete:", output)
