...
class PowerFlowSimulation(SimulationBase):
    """Deterministic classical simulation for Power Flow."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Power Flow",
            parameters={"problem_size": 128, "seed": 62949, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 62949))
...
if __name__ == "__main__":
    output = main()
    print("PowerFlowSimulation complete:", output)
