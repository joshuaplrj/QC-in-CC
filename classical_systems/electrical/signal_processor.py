...
class SignalProcessorSimulation(SimulationBase):
    """Deterministic classical simulation for Signal Processor."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Signal Processor",
            parameters={"problem_size": 128, "seed": 47628, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 47628))
...
def main() -> dict[str, Any]:
    simulation = SignalProcessorSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("SignalProcessorSimulation complete:", output)
