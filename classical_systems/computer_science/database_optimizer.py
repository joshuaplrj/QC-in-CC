...
class DatabaseOptimizerSimulation(SimulationBase):
    """Deterministic classical simulation for Database Optimizer."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Database Optimizer",
            parameters={"problem_size": 128, "seed": 29525, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 29525))
...
def main() -> dict[str, Any]:
    simulation = DatabaseOptimizerSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("DatabaseOptimizerSimulation complete:", output)
