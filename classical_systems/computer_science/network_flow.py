...
class NetworkFlowSimulation(SimulationBase):
    """Deterministic classical simulation for Network Flow."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Network Flow",
            parameters={"problem_size": 128, "seed": 4801, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 4801))
...
def main() -> dict[str, Any]:
    simulation = NetworkFlowSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("NetworkFlowSimulation complete:", output)
