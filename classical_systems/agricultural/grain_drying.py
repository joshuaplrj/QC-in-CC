...
class GrainDryingSimulation(SimulationBase):
    """Deterministic classical simulation for Grain Drying."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Grain Drying",
            parameters={"problem_size": 128, "seed": 54426, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 54426))
...
def main() -> dict[str, Any]:
    simulation = GrainDryingSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("GrainDryingSimulation complete:", output)
