...
class CropGrowthSimulation(SimulationBase):
    """Deterministic classical simulation for Crop Growth."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Crop Growth",
            parameters={"problem_size": 128, "seed": 58764, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 58764))
...
def main() -> dict[str, Any]:
    simulation = CropGrowthSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("CropGrowthSimulation complete:", output)
