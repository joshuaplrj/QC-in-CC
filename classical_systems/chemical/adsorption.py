...
class AdsorptionSimulation(SimulationBase):
    """Deterministic classical simulation for Adsorption."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Adsorption",
            parameters={"problem_size": 128, "seed": 53603, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 53603))
...
def main() -> dict[str, Any]:
    simulation = AdsorptionSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("AdsorptionSimulation complete:", output)
