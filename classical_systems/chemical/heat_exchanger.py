...
class HeatExchangerSimulation(SimulationBase):
    """Deterministic classical simulation for Heat Exchanger."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Heat Exchanger",
            parameters={"problem_size": 128, "seed": 50438, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 50438))
...
if __name__ == "__main__":
    output = main()
    print("HeatExchangerSimulation complete:", output)
