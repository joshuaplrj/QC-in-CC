...
class ReactorKineticsSimulation(SimulationBase):
    """Deterministic classical simulation for Reactor Kinetics."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Reactor Kinetics",
            parameters={"problem_size": 128, "seed": 48317, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 48317))
...
def main() -> dict[str, Any]:
    simulation = ReactorKineticsSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("ReactorKineticsSimulation complete:", output)
