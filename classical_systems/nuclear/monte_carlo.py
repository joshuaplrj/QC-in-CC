...
class MonteCarloSimulation(SimulationBase):
    """Deterministic classical simulation for Monte Carlo."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Monte Carlo",
            parameters={"problem_size": 128, "seed": 90667, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 90667))
...
if __name__ == "__main__":
    output = main()
    print("MonteCarloSimulation complete:", output)
