...
class WaterNetworkSimulation(SimulationBase):
    """Deterministic classical simulation for Water Network."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Water Network",
            parameters={"problem_size": 128, "seed": 19003, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 19003))
...
def main() -> dict[str, Any]:
    simulation = WaterNetworkSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("WaterNetworkSimulation complete:", output)
