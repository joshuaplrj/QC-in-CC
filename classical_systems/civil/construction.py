...
class ConstructionSimulation(SimulationBase):
    """Deterministic classical simulation for Construction."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Construction",
            parameters={"problem_size": 128, "seed": 3587, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 3587))
...
if __name__ == "__main__":
    output = main()
    print("ConstructionSimulation complete:", output)
