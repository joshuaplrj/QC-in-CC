...
class AircraftWeightSimulation(SimulationBase):
    """Deterministic classical simulation for Aircraft Weight."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Aircraft Weight",
            parameters={"problem_size": 128, "seed": 7496, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 7496))
...
def main() -> dict[str, Any]:
    simulation = AircraftWeightSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("AircraftWeightSimulation complete:", output)
