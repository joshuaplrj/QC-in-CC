...
class PanelMethodSimulation(SimulationBase):
    """Deterministic classical simulation for Panel Method."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Panel Method",
            parameters={"problem_size": 128, "seed": 17814, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 17814))
...
def main() -> dict[str, Any]:
    simulation = PanelMethodSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("PanelMethodSimulation complete:", output)
