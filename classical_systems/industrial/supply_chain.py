...
class SupplyChainSimulation(SimulationBase):
    """Deterministic classical simulation for Supply Chain."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Supply Chain",
            parameters={"problem_size": 128, "seed": 25200, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 25200))
...
if __name__ == "__main__":
    output = main()
    print("SupplyChainSimulation complete:", output)
