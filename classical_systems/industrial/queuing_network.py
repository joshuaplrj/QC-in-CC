...
class QueuingNetworkSimulation(SimulationBase):
    """Deterministic classical simulation for Queuing Network."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Queuing Network",
            parameters={"problem_size": 128, "seed": 50712, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 50712))
...
def main() -> dict[str, Any]:
    simulation = QueuingNetworkSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("QueuingNetworkSimulation complete:", output)
