...
class StatisticalControlSimulation(SimulationBase):
    """Deterministic classical simulation for Statistical Control."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Statistical Control",
            parameters={"problem_size": 128, "seed": 3498, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 3498))
...
def main() -> dict[str, Any]:
    simulation = StatisticalControlSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("StatisticalControlSimulation complete:", output)
