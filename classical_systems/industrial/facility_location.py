...
class FacilityLocationSimulation(SimulationBase):
    """Deterministic classical simulation for Facility Location."""

    def __init__(self, config: SimulationConfig | None = None) -> None:
        config = config or SimulationConfig(
            name="Facility Location",
            parameters={"problem_size": 128, "seed": 10572, "scale": 1.0},
        )
        super().__init__(config)

    def setup(self) -> None:
        self.problem_size = max(8, int(self.parameters.get("problem_size", 128)))
        self.seed = int(self.parameters.get("seed", 10572))
...
def main() -> dict[str, Any]:
    simulation = FacilityLocationSimulation()
    return simulation.run()


if __name__ == "__main__":
    output = main()
    print("FacilityLocationSimulation complete:", output)
